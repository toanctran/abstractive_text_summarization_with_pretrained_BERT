...
def _create_topk_unique(inputs, k):
    ...
    for i in range(k):
        kth_order_statistic = tf.reduce_max(
            input_tensor=tmp, axis=1, keepdims=True)
        k_mask = tf.tile(tf.expand_dims(tf.equal(tf.range(k), tf.fill([k], i)), 0),
                         [height, 1])
        topk_r2 = tf.compat.v1.where(k_mask, tf.tile(
            kth_order_statistic, [1, k]), topk_r2)
        ge_r2 = tf.greater_equal(inputs, tf.tile(
            kth_order_statistic, [1, width]))
...
def compute_topk_scores_and_seq(sequences,
    ...
    if not use_tpu:
        ...
        # it a concrete name to simplify observing these operations with tfdbg.
        # Clients can capture these tensors by watching these node names.
        topk_seq = fast_tpu_gather(
            sequences, topk_indexes, prefix + "_topk_seq")
        topk_flags = fast_tpu_gather(
            flags, topk_indexes, prefix + "_topk_flags")
...
def beam_search(symbols_to_logits_fn,
    ...
    def grow_topk(i, alive_seq, alive_log_probs, states):
        ...
        length_penalty = tf.pow(
            ((5. + tf.cast(i + 1, dtype=tf.float32)) / 6.), alpha)
        ...
        if use_tpu and use_top_k_with_unique:
            ...
            topk_scores, topk_ids = tf.nn.top_k(
                flat_curr_scores, k=beam_size * 2)
    ...
    def _is_not_finished(i, unused_alive_seq, alive_log_probs,
        ...
        max_length_penalty = tf.pow(
            ((5. + tf.cast(decode_length, dtype=tf.float32)) / 6.), alpha)
    ...
    if use_tpu:
        state_struc = nest.map_structure(
            lambda state: state.get_shape(), states)
    else:
