...
from layers.transformer import create_masks, Decoder
...
def _embedding_from_bert():
    ...
    vocab_of_BERT = TFBertModel.from_pretrained(
        'bert-base-uncased', trainable=False)
...
class AbstractiveSummarization(tf.keras.Model):
    ...
    def refine_summary(self,
        ...
        refine_logits = tf.concat(
            [tf.tile(tf.expand_dims(tf.one_hot([CLS_ID], self.vocab_size), axis=0), [
                     N, 1, 1]), dec_outputs],
            axis=1
        )
