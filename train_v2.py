...
from ops.create_tokenizer import tokenizer, model
from ops.creates import log, train_summary_writer, valid_summary_writer
from ops.input_path import file_path
from ops.metrics import optimizer, loss_function, label_smoothing, get_loss_and_accuracy, tf_write_summary, monitor_run
...
def train_step(input_ids,
    ...
    with tf.GradientTape() as tape:
        ...
        draft_summary_loss = loss_function(
            target_ids[:, 1:, :], draft_predictions, mask)
        refine_summary_loss = loss_function(
            target_ids[:, :-1, :], refine_predictions, mask)
    ...
    if not accumulators:
        for tv in gradients:
            accumulators.append(tf.Variable(
                tf.zeros_like(tv), trainable=False))
    ...
    if grad_accum_flag:
        for accumlator in accumulators:
            accumulator.assign(tf.math.divide(
                accumulator, h_parms['accumulation_steps']))
...
def val_step(input_ids,
    ...
    if create_summ:
        rouge, bert = tf_write_summary(
            target_ids_[:, :-1], refine_predictions, epoch)
    else:
        rouge, bert = (1.0, 1.0)
    return (rouge, bert)
...
for epoch in range(h_parms['epochs']):
    ...
    for (batch, (input_ids, input_mask, input_segment_ids, target_ids_, target_mask, target_segment_ids)) in enumerate(train_dataset):
        ...
        if batch > config['start_from_batch']:
            ...
            target_ids = label_smoothing(tf.one_hot(
                target_ids_, depth=config['input_vocab_size']))
            grad_accum_flag = True if (
                batch+1) % h_parms['accumulation_steps'] == 0 else False
            train_step(
                input_ids,
                input_mask,
                input_segment_ids,
